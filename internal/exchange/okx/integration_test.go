...
func TestIntegrationOKXAccountBalance(t *testing.T) {
	integration.RequireIntegration(t)

	module := NewTradingModule(
		integration.RequireEnv(t, "OKX_API_KEY"),
		integration.RequireEnv(t, "OKX_SECRET_KEY"),
		integration.RequireEnv(t, "OKX_PASSPHRASE"),
		true,
	)
	...
	body, err := module.sendRequest(ctx, "GET", "/api/v5/account/balance", nil)
	if err != nil {
		t.Fatalf("account balance request error = %v", err)
	}
	if len(body) == 0 {
		t.Fatal("account balance request returned an empty body")
	}
}

func TestIntegrationOKXMarketTicker(t *testing.T) {
	integration.RequireIntegration(t)

	ticker := waitOKXTicker(t)
...
func TestIntegrationOKXGetInstrument(t *testing.T) {
	integration.RequireIntegration(t)
	...
	instrument, err := module.GetInstrument(ctx, "BTC-USDT-SWAP")
	if err != nil {
	...
	t.Logf("OKX instrument: symbol=%s lotSize=%s baseLotSize=%s ctVal=%s stepSize=%s baseStepSize=%s tickSize=%s", instrument.Symbol, instrument.LotSize, instrument.BaseLotSize, instrument.CtVal, instrument.StepSize, instrument.BaseStepSize, instrument.TickSize)
}

func TestIntegrationOKXGetOHLCV(t *testing.T) {
	integration.RequireIntegration(t)
	...
	candles, err := module.GetOHLCV(ctx, "BTC-USDT-SWAP", "1m", 2)
	if err != nil {
	...
	t.Logf("OKX candle: symbol=%s timeframe=%s ts=%d open=%s close=%s volume=%s", candles[0].Symbol, candles[0].Timeframe, candles[0].Timestamp, candles[0].Open, candles[0].Close, candles[0].Volume)
}

func TestIntegrationOKXPlaceAndCancelOrder(t *testing.T) {
	integration.RequireIntegration(t)
	...
	waitOpenOrder(t, ctx, module, "BTC-USDT-SWAP", clientOrderID)
	...
	if err := module.CancelOrder(ctx, "BTC-USDT-SWAP", clientOrderID); err != nil {
		t.Fatalf("CancelOrder() error = %v", err)
	}
}

func TestIntegrationOKXPlaceTrailingOrder(t *testing.T) {
	integration.RequireIntegration(t)
	...
	order, err := module.PlaceTrailingOrder(ctx, models.TrailingOrderReq{
		...
		CallbackSpread:  ticker.AskPrice.Mul(decimal.RequireFromString("0.05")).Round(1),
	})
	if err != nil {
	...
	}, "OKX injected trailing order create")
	waitOpenAlgoOrder(t, ctx, module, "BTC-USDT-SWAP", clientOrderID)
	if err := module.CancelTPSL(ctx, "BTC-USDT-SWAP", models.TPSLOrder{TakeProfitOrderID: order.OrderID}); err != nil {
		t.Fatalf("Cancel trailing order error = %v", err)
	}
}

func TestIntegrationOKXFullTradingLifecycle(t *testing.T) {
	integration.RequireIntegration(t)
	...
	waitIntegrationStep(t, "OKX account configuration")
	...
	}, "OKX open market fill")
	waitOrderCallback(t, openOnce, func(update *models.OrderUpdate) bool {
		return update.ClientOrderID == openClientID && update.Status == models.OrderStatusFilled
	}, "OKX injected open market fill")
	waitPositionCallback(t, positionUpdates, func(position *models.Position) bool {
		return position.Exchange == "okx" && position.Symbol == "BTC-USDT-SWAP" && position.PositionSide == "LONG" && position.PositionAmt.Abs().GreaterThanOrEqual(decimal.RequireFromString("0.01"))
	}, "OKX open position update")
	waitBalanceCallback(t, balanceUpdates, func(balance *models.AccountBalance) bool {
		return balance.Exchange == "okx" && balance.Asset == "USDT"
	}, "OKX balance update")
	waitIntegrationStep(t, "OKX open callbacks and account updates")

	position := waitPosition(t, ctx, module, "BTC-USDT-SWAP", models.PositionSideLong, decimal.RequireFromString("0.01"))
	tpslReq := okxTPSLFromPosition(position, "cstpsl")
	...
	}, "OKX injected take-profit create")
	...
	}, "OKX injected stop-loss create")
	waitOpenAlgoOrder(t, ctx, module, "BTC-USDT-SWAP", tpsl.TakeProfitClientOrderID)
	waitOpenAlgoOrder(t, ctx, module, "BTC-USDT-SWAP", tpsl.StopLossClientOrderID)
	waitIntegrationStep(t, "OKX initial take-profit and stop-loss callbacks")
	...
	waitOrderCallback(t, addOnce, func(update *models.OrderUpdate) bool {
		return update.ClientOrderID == addClientID && update.Status == models.OrderStatusFilled
	}, "OKX injected add market fill")
	waitPositionCallback(t, positionUpdates, func(position *models.Position) bool {
		return position.Exchange == "okx" && position.Symbol == "BTC-USDT-SWAP" && position.PositionSide == "LONG" && position.PositionAmt.Abs().GreaterThanOrEqual(decimal.RequireFromString("0.02"))
	}, "OKX add position update")
	waitIntegrationStep(t, "OKX add callbacks and position quantity update")

	position = waitPosition(t, ctx, module, "BTC-USDT-SWAP", models.PositionSideLong, decimal.RequireFromString("0.02"))
	updateReq := okxTPSLFromPosition(position, "csutpsl")
	...
	}, "OKX injected updated take-profit create")
	...
	}, "OKX injected updated stop-loss create")
	waitIntegrationStep(t, "OKX refreshed take-profit and stop-loss callbacks")
	...
	if err := module.CancelTPSL(ctx, "BTC-USDT-SWAP", updatedTPSL); err != nil {
	...
	}, "OKX injected take-profit cancel")
	...
	}, "OKX injected stop-loss cancel")
	waitIntegrationStep(t, "OKX take-profit and stop-loss cancel callbacks")

	closeOrderID, err := module.ClosePosition(ctx, "BTC-USDT-SWAP", models.PositionSideLong)
	if err != nil {
	...
	}, "OKX close market fill")
	waitPositionCallback(t, positionUpdates, func(position *models.Position) bool {
		return position.Exchange == "okx" && position.Symbol == "BTC-USDT-SWAP" && position.PositionSide == "LONG" && position.PositionAmt.IsZero()
	}, "OKX close position update")
	waitIntegrationStep(t, "OKX close callbacks and flat position")
}

func okxTPSLFromPosition(position *models.Position, idPrefix string) models.TPSLReq {
...
func waitOKXTicker(t *testing.T) *models.Ticker {
	t.Helper()
	...
	if err := module.SubscribeTickers([]string{"BTC-USDT-SWAP"}); err != nil {
	...
	case <-time.After(15 * time.Second):
		t.Fatal("timed out waiting for OKX ticker")
	}
	return nil
}
